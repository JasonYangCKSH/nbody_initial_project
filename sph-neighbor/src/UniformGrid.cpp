...
CellId UniformGrid::cellIdFromPos(const Eigen::Vector3d &p) const {
    ...
    int ix = static_cast<int>(std::floor(rel.x() / cfg_.cellSideLength));
    int iy = static_cast<int>(std::floor(rel.y() / cfg_.cellSideLength));
    int iz = static_cast<int>(std::floor(rel.z() / cfg_.cellSideLength));
...
void UniformGrid::findNeighbors(const std::vector<Particle> &particles,
    ...
    // 預先算 cell 計數
    int r = static_cast<int>(std::ceil(search_radius / cfg_.cellSideLength));
    ...
    for (int i = 0; i < (int)particles.size(); i++) {
        ...
        int ix = (int)std::floor(rel.x() / cfg_.cellSideLength);
        int iy = (int)std::floor(rel.y() / cfg_.cellSideLength);
        int iz = (int)std::floor(rel.z() / cfg_.cellSideLength);

        std::vector<int> &neigh_list = neighbors[i];
