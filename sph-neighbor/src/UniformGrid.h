...
struct GridConfig {
    double cellSideLength;  // cell side length
    Eigen::Vector3d origin;  // grid origin position (原點)
...
private:
    GridConfig cfg_;
    std::unordered_map<CellId, std::vector<int>> cell_map_;  // cell_id -> particle indices(index)
