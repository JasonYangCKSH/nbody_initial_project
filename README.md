...
(2) 空域擁擠度：某航道/高度層是否過密
(3) 編隊控制近似互動：對每台 UAV 計算「遠處群集的影響」（吸引/排斥/隊形保持）──這非常像 BH 的力計算
* BH-Octree功能
(1) 近鄰/避碰：走訪時若某 cell 的 bounding cube 與 UAV 的查詢球不相交直接剪枝；cell 太大/太近就下探
(2) 群集影響：遠處 cell 直接用 COM + count + avg_vel 當成「一個群」近似（θ 控制精度/速度）
