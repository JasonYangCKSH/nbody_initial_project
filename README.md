...
### 2026/3/11
1. octree+AABB粗略篩選+triangle碰撞精算方法的論文
* 簡化為(球心、半徑)表示的球體物件時，雖然octree+AABB仍有助於效率提升，但是二球體是否碰撞不需要耗時的triangle碰撞精算，換成grid結構直接算同格或鄰近方格的球體或許更有效率。
* 提出一個應用情境來結合二種需求，以便採用同一棵octree來計算是否碰撞及近似遠距重力。
2. 更新應否重建octree或局部更新的問題
* 因計算近似遠距重力的成本遠高於重建，一旦重建可以簡化，再加上GPU平行化加速，可能就沒必要局部更新。
* 計算近似遠距重力適合整批block timestep更新的重建方式，可能是更好的折衷，因為並非複雜而步驟瑣碎的局部更新，也有機會採用平行化加速。
* 計算是否碰撞可以考慮loose octree或dynamic BVH，可能是建立另一棵樹，但是採取局部更新而非重建。
### 專題方向
...
* 範例
  ...
  - 若你採「可分裂/合併的動態樹」：
    - 多數星體每步移動很小 → 很多仍留在原 leaf 或鄰近 leaf
    - 只對跨界的星體做 reinsert；星團核心可能局部 split 更深、外圍 merge 更淺
    - 當整體分布漂移太大（例如星系合併）才需要重建
* **4-2. 查詢需求**（即時查詢）
* 對每個星體計算引力加速度（核心）
...
### 參考文獻
1. [TOG 2022] Fast Octree Neighborhood Search for SPH Simulations [publication](https://dl.acm.org/doi/10.1145/3550454.3555523)
* 簡介
> 這篇論文的研究課題是加速 SPH 模擬中最關鍵也最昂貴的固定半徑近鄰搜尋。既有主流方法多依賴均勻格或以固定格寬建立候選鄰居集合，但在粒子分佈高度不均勻、或不同粒子擁有不同支撐半徑的情境下，均勻格容易產生大量不必要的候選比對，導致效能與可擴展性受限。為了讓近鄰搜尋能隨密度自動調整，作者提出以八叉樹作為自適應空間索引，高密度區域細分、低密度區域保持粗分，使每次查詢只需走訪必要的樹節點來取得更緊的候選集合。方法上，他們不只把重點放在如何找到候選粒子，也把核心貢獻放在如何讓後續的距離過濾更符合硬體特性：透過空間排序讓記憶體配置更貼近空間鄰近性，並將查詢工作以較大的批次處理，讓計算更容易向量化且提升快取命中率，避免每個粒子各自零碎掃描造成的記憶體瓶頸。實驗顯示，這套八叉樹近鄰搜尋在多種 SPH 場景中相較於既有均勻格導向方法能帶來顯著加速，尤其在多解析度或半徑不一的設定下更具優勢，因為它不需要以全域最大半徑去保守地擴大搜尋範圍，而是能以局部結構自適應地控制候選數量與走訪成本。
* 評論
> 適合「大量物件、每步大量近鄰查詢」的動態模擬，不太適合「每步更新很頻繁但查詢量不大」的場景。
---
---
1/14-1/20
